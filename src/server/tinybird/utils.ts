...
export async function calculateViewCounts(
  ...
  getSectionDuration: (params: { link_id: string }) => Promise<{ data: SectionData[] }>
) {
  ...
  for (const link of portalData.links) {
    ...
    const filteredData = data.data.filter((item) => item.link_id === link.linkId);

    // Create a Set to track unique session IDs
    const uniqueSessionIds = new Set(filteredData.map((item) => item.session_id));
...
export function averageDurationData(
  ...
  if (!tinybirdData || !Array.isArray(tinybirdData.data)) {
      console.error("Expected an array for tinybirdData.data, but received:", tinybirdData);
  ...
  const tinybirdDurationMap: Record<string, { section_title: string; total_duration: number }> = {};
  ...
  tinybirdData.data.forEach((item: ExtendedSectionData) => {
      tinybirdDurationMap[item.section_id] = {
          ...
          total_duration: item.average_duration ? item.average_duration / 1000 : 0, // Convert to seconds
      };
  });

  // Iterate over the portal sections to create the merged data
  const mergedData: MergedSectionData[] = portalData.sections.map((section) => {
...
export function topEngagedData(
  ...
  const mergedData: MergedEngagedData[] = tinybirdData.data
    .filter((item: { link_id: string; }) => {
      ...
      return portalData.links.some((link) => link.linkId === item.link_id);
    })
    .map((item: { link_id: string; portal_id: any; user_id: any; total_duration: any; last_view_timestamp: any; }) => {
      // Find the corresponding link and candidate
      const link = portalData.links.find((link) => link.linkId === item.link_id);
      const candidateName = link ? candidateNameMap[link.candidateId] : "Unknown Candidate";

      return {
        link_id: item.link_id,
        portal_id: item.portal_id,
        user_id: item.user_id,
        total_duration: item.total_duration ? item.total_duration / 1000 : 0, // Convert to seconds
